<script lang="ts">
	...
	/** Which bot's real player screen the tab strip is showing. */
	...
	async function build() {
		...
		try {
			...
			botSessions = made;
			bots = made;
			activeSeatId = made[0]?.mySeat?.id ?? null;
...
<div class="stack simulator">
	...
	<section class="card row" style="align-items:flex-end">
	...
	{#if st && st.game}
		...
		{#if dealtSeats.length}
			<section class="card stack">
				...
				<p class="muted" style="margin:0">
					...
					have to click through every bot tab just to check what a role says.
				</p>
		...
		{#if bots.length}
			<section class="card stack">
				<strong>Player screens</strong>
				<p class="muted" style="margin:0">
					Each tab is that seat's actual phone screen — click through to test the real UI, not
					just a summary.
				</p>
				<nav class="bottabs">
					{#each bots as b (b.mySeat?.id ?? b)}
						...
						<button
							class="bottab"
							class:active={activeSeatId === b.mySeat?.id}
							class:dead
							onclick={() => (activeSeatId = b.mySeat?.id ?? null)}
						>
				...
				{#each bots as b (b.mySeat?.id ?? b)}
					<div hidden={activeSeatId !== b.mySeat?.id} class="botscreen">
